...
void Console::cmdSetState(const std::vector<std::string>& args)
{
    ...
    // int32s) or TWO board blocks (2 * STATE_CODEC_SIZE -> [mover board,
    // opponent board]). Both are the flat serialization from codec::serialize /
    // the WASM frontend. The web frontend sends the opponent's board whenever
    // there is one; this single-player env plays the mover's board only, so the
    // opponent block is accepted and ignored.
...
void Console::cmdGetAiInfo(const std::vector<std::string>& args)
{
    ...
    // just injected via set_state. Currently: value (env value head).
    if (!checkArgument(args, 1, 1)) { return; }
#if MODERNTETRIS_PLACEMENT
    using namespace minizero::env::moderntetris_placement;
